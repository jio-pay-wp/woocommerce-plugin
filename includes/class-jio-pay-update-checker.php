...
class Jio_Pay_Update_Checker {
    ...
    public function check_for_update($transient) {
        ...
        if ($remote_info && version_compare($this->version, $remote_info['version'], '<')) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Jio Pay Update Available: ' . $this->version . ' -> ' . $remote_info['version']);
                error_log('Download URL: ' . $remote_info['download_url']);
            }
    ...
    private function get_remote_version() {
        ...
        if (isset($data['tag_name'])) {
            // Check if there's a ZIP asset uploaded, otherwise use zipball_url
            $download_url = $data['zipball_url'];
            
            // Prefer uploaded ZIP asset over automatic GitHub zipball
    ...
    public function after_install($response, $hook_extra, $result) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Jio Pay Update: Post-install triggered');
            error_log('Destination: ' . print_r($result['destination'], true));
        }
        ...
        if ($wp_filesystem->move($result['destination'], $install_directory)) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Jio Pay Update: Files moved successfully to ' . $install_directory);
            }
        } else {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Jio Pay Update: Failed to move files to ' . $install_directory);
            }
        }
